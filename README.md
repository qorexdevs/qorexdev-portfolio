...
Публичный адрес: [https://qorex.dev](https://qorex.dev). Три демо работают в браузере; меню [демонстрационного бота](https://t.me/qorexdev_orderly_bot) открывает Orderly внутри Telegram. Для ответа на сообщения боту нужен Worker из раздела ниже. Компании, контакты и операции внутри демо вымышлены; реальные платежи и уведомления о заказах отключены.
...
Стек: React, TypeScript, Vite, Express 5, Node.js 24 и SQLite. Один Node-процесс на VPS обслуживает интерфейс, API и фоновые проверки. Caddy отвечает за HTTPS. Для ответа бота подготовлен отдельный Cloudflare Worker: Telegram не доставляет webhook напрямую на московский VPS. Демоданные остаются в SQLite на VPS.
...
1. Создать собственного бота через BotFather и обеспечить общедоступный HTTPS-адрес Mini App: `https://ВАШ-ДОМЕН/demo/orderly`.
2. На сервере задать TELEGRAM_BOT_TOKEN в .env и пересоздать app через Compose. Создать случайный TELEGRAM_WEBHOOK_SECRET из 16-128 символов `A-Z`, `a-z`, `0-9`, `_` или `-`.
3. Войти в Cloudflare и развернуть `relay/worker.mjs` через `npx wrangler@4.141.0 deploy --config relay/wrangler.jsonc`. Командой `npx wrangler@4.141.0 secret put TELEGRAM_WEBHOOK_SECRET --config relay/wrangler.jsonc` записать тот же секрет в серверное хранилище Worker. В код Worker не помещать ни секрет, ни токен бота. Получить адрес вида `https://qorexdev-orderly-bot.<аккаунт>.workers.dev`.
4. Через Bot API установить `setChatMenuButton` с типом `web_app` и URL Mini App. Зарегистрировать `setWebhook` на `<АДРЕС_WORKER>/telegram/webhook` с `secret_token`, `allowed_updates=["message"]`, `max_connections=1`, без фиксированного IP московского VPS. Через `getWebhookInfo` проверить адрес и отсутствие ошибок доставки. Для разовой регистрации нужен доступ к Bot API; Worker работает без исходящего прокси и не хранит бот-токен.
5. При сообщении боту в личном чате Worker возвращает кнопку запуска. Mini App передает initData на /api/session/telegram; сервер на VPS проверяет HMAC-SHA-256, возраст auth_date до 5 минут, отсутствие дублирующихся параметров и корректный идентификатор пользователя.

Токен и секрет webhook никогда не включаются во frontend bundle. Не задавайте секреты в VITE_* переменных. Подпись меняет режим сессии на telegram, но оформление остается тестовым, роли остаются демонстрационными, реальные платежи и уведомления покупателям отключены. Встроенный Telegram Web с iframe может ограничивать сторонние cookies; этот режим требует отдельной проверки в клиенте Telegram. Открытие в обычном браузере остается доступным.
