...
Временный публичный адрес: [http://185.182.65.214](http://185.182.65.214). Пока домен не подключен, три демо работают в браузере по HTTP; запуск Orderly внутри Telegram потребует HTTPS. Компании, контакты и операции внутри демо вымышлены; реальные платежи и внешние сообщения не отправляются.
...
3. Скопировать .env.example в .env. Задать DOMAIN, PUBLIC_ORIGIN и ACME_EMAIL. Например DOMAIN=portfolio.example.com и PUBLIC_ORIGIN=https://portfolio.example.com. PUBLIC_ORIGIN содержит только origin, без пути и слеша в конце.
   Для временного запуска по IP допустимы DOMAIN=http://185.182.65.214 и PUBLIC_ORIGIN=http://185.182.65.214. Это только браузерное демо; для Telegram Mini App нужен HTTPS-домен.
