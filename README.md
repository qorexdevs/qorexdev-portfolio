...
Публичный адрес: [https://qorex.dev](https://qorex.dev). Три демо работают в браузере; меню [демонстрационного бота](https://t.me/qorexdev_orderly_bot) открывает Orderly внутри Telegram. Бот получает сообщения с VPS и отвечает кнопкой запуска через локальный Tor-клиент. Компании, контакты и операции внутри демо вымышлены; реальные платежи и уведомления о заказах отключены.
...
Стек: React, TypeScript, Vite, Express 5, Node.js 24 и SQLite. Один Node-процесс на VPS обслуживает интерфейс, API, фоновые проверки и polling бота. Caddy отвечает за HTTPS. Исходящие запросы к Telegram Bot API проходят через локальный Tor-клиент со Snowflake-мостом; отдельный хостинг для бота не требуется. Позиция последнего обработанного обновления хранится в SQLite.
...
2. Создать A-запись корня домена на IPv4 VPS и CNAME `www` на корень. Удалить старые A/AAAA-записи сайта; AAAA добавлять только при настроенном IPv6. Почтовые MX/TXT сохранять. Открыть входящие TCP 80/443; UDP 443 нужен для HTTP/3.
3. Скопировать .env.example в .env. Задать DOMAIN, WWW_DOMAIN, PUBLIC_ORIGIN и ACME_EMAIL. Например DOMAIN=portfolio.example.com, WWW_DOMAIN=www.portfolio.example.com и PUBLIC_ORIGIN=https://portfolio.example.com. PUBLIC_ORIGIN содержит только origin, без пути и слеша в конце.
...
Caddy автоматически получает TLS-сертификат для корня и `www`, когда DNS указывает на VPS и порты доступны. `www` перенаправляется на основной адрес. Приложение слушает 3000 только внутри сети Compose; публично открыты 80/443 Caddy. Путь /health проверяет доступность БД и показывает время последнего прохода фонового процесса.
...
Без ключей Orderly работает в браузерном деморежиме. Для запуска через Telegram:

1. Создать собственного бота через BotFather и обеспечить общедоступный HTTPS-адрес Mini App: `https://ВАШ-ДОМЕН/demo/orderly`. Через Bot API установить меню с типом `web_app` и этим URL.
2. На VPS установить `tor` и `snowflake-client`. Настроить Tor как клиент со Snowflake-мостом по [инструкции Tor Project](https://support.torproject.org/little-t-tor/circumvention/using-bridges/); проверенный для этого VPS фрагмент настроек лежит в `deploy/torrc.snowflake`. Перед применением сверить адрес Docker host-gateway и подсеть проекта через `docker network inspect qorexdev-portfolio_default`: оба адреса в примере привязаны к текущему VPS. Убедиться в `Bootstrapped 100%` и проверить `curl --socks5-hostname 127.0.0.1:9050 https://api.telegram.org/`. Публичный SOCKS-порт не открывать.
3. Удалить прежний Telegram webhook через `deleteWebhook` без сброса ожидающих обновлений: `getUpdates` и webhook одновременно не работают. На сервере задать TELEGRAM_BOT_TOKEN, TELEGRAM_BOT_POLLING=1 и BOT_PROXY_URL=socks5h://host.docker.internal:9050 в .env; пересоздать app через Compose.
4. Node-процесс получает новые личные сообщения через `getUpdates`, отправляет кнопку Orderly через `sendMessage` и сохраняет offset в SQLite. При сетевой ошибке повторяет запрос с ограниченной паузой, не подтверждая ещё не обработанное сообщение. Mini App передает initData на /api/session/telegram; сервер проверяет HMAC-SHA-256, возраст auth_date до 5 минут, отсутствие дублирующихся параметров и корректный идентификатор пользователя.

Токен никогда не включается во frontend bundle. Не задавайте секреты в VITE_* переменных. Доступность ответов бота зависит от сети Tor/Snowflake; интерфейс портфолио и три демо на VPS работают независимо от неё. Подпись меняет режим сессии на telegram, но оформление остается тестовым, роли остаются демонстрационными, реальные платежи и уведомления покупателям отключены. Встроенный Telegram Web с iframe может ограничивать сторонние cookies; этот режим требует отдельной проверки в клиенте Telegram. Открытие в обычном браузере остается доступным.
